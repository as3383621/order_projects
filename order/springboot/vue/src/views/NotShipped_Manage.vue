            ...
            <el-table-column label="操作" width="170" align="center">
                <template slot-scope="scope">
                    <el-popconfirm
                            ...
                            icon-color="red"
                            title="确认出餐吗？"
                            @confirm="meal(scope.row.id)"
                    >
                        <el-button type="success" slot="reference">出餐</el-button>
                    </el-popconfirm>
                </template>
            </el-table-column>
        </el-table>
        ...
        methods: {
            ...
            load() {
                fetch("http://localhost:9090/shipped/findpage?pageNum=" + this.pageNum + "&pageSize=" + this.pageSize +
                    "&username=" + this.username +"&FoodName=" + this.FoodName + "&state=待出餐",{
                    headers: {
                        token: this.token,
                        author: "root"
                    }
            ...
            meal(id) {
                fetch("http://localhost:9090/shipped/meal?id=" + id,{
                    headers: {
                        token: this.token,
                        author: "root"
                    }
